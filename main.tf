...
data "cloudinit_config" "minikube_cloud_init" {
  ...
  part {
    ...
    content = templatefile("${path.module}/scripts/init-aws-minikube.sh", { kubeadm_token = module.kubeadm-token.token, dns_name = "${var.cluster_name}.${var.env}.${var.hosted_zone}", ip_address = aws_eip.minikube.public_ip, cluster_name = var.cluster_name, kubernetes_version = var.kubernetes_version, addons = join(" ", var.addons) } )
  }
}
...
data "aws_ami" "centos7" {
  ...
  filter {
    name = "product-code"
    values = ["eh5mv20mqy0oiwth3ngqg8qsl"]
  }
